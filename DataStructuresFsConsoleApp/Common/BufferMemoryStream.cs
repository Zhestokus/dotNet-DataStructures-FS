...
{
    public class BufferMemoryStream : Stream
    {
        ...
        //private long _memPos;
        private readonly Stream _memoryStream;

        public BufferMemoryStream(Stream baseStream)
            : this(baseStream, 4096)
        {
        }

        public BufferMemoryStream(Stream baseStream, int capacity)
            : this(baseStream, capacity, 4096)
        {
        }

        public BufferMemoryStream(Stream baseStream, int capacity, int bufferSize)
        {
        ...
        public override void Flush()
        {
            ...
            if (_memoryStream.Length > 0L)
            {
                ...
                _memoryStream.CopyTo(_baseStream, _bufferSize);
