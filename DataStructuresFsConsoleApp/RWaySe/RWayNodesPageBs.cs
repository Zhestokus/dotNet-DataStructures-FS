...
{
    public class RWayNodesPageBs<TKey, TValue> : IEnumerable<RWayNodeBs<TKey, TValue>>
    {
        ...
        public RWayNodesPageBs(long position, Stream stream, IFormatter keySerializer, IFormatter valueSerializer)
        {
            _position = position;
            _stream = stream;
        ...
        private void Init()
        {
            ...
            else
            {
                ...
                for (int i = 0; i < Size; i++)
                    _childPositions[i] = BitConverter.ToInt64(bytes, i * 8);
        ...
        private bool NeedFlush()
        {
            ...
            {
                var child = _children[i];
                var position = _childPositions[i];

                var childPos = (child == null ? position : child.Position);

                if (childPos != position)
                {
                    _childPositions[i] = childPos;
